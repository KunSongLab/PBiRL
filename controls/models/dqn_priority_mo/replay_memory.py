...
class MAPrioritizedReplayMemory(object):
    ...
    def push(self, *args):
        ...
        for agent_id in self.agent_ids:
            ...
            # 从各输入中取出该 agent 对应的数据
            agent_obs = args[0][agent_id]
            agent_action = args[1][agent_id]
            agent_next_obs = args[2][agent_id]
            agent_reward = args[3][agent_id]
            agent_reward_speed = agent_reward["speed"]
            agent_reward_toll = agent_reward["toll"]
            trans = Transition(agent_obs, agent_action, agent_next_obs, agent_reward)
            self.memories[agent_id].push(trans)
            self.it_sums[agent_id][temp_idx] = self.max_priority[agent_id] ** self.alpha
            self.it_mins[agent_id][temp_idx] = self.max_priority[agent_id] ** self.alpha
