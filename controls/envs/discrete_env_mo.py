...
class MonstacEnv(MultiAgentEnv):
    ...
    def sigmoid_func(self, x, k=6, x0=0.6):
        sigmoid = lambda z: 1 / (1 + np.exp(-z))
        return (sigmoid(k * (x - x0)) - sigmoid(-k * x0)) / (sigmoid(k * (1 - x0)) - sigmoid(-k * x0))
    ...
    def step(self, action):
        ...
        for key, value in action.items():
            if value == 0:
                self.actions[key] = max(min(self.actions[key] - 0.25, 1), 0)
            elif value == 2:
                self.actions[key] = max(min(self.actions[key] + 0.25, 1), 0)
        done = self.base_env.step(self.actions)
        temp_reward = {}
        for idx, agent_id in enumerate(self.agent_ids):
            temp_reward[agent_id] = {
                "toll": monstac_api.rl.get_rewards(self.base_env)[0][idx],
                "speed": monstac_api.rl.get_rewards(self.base_env)[1][idx],
            }
        # observation, reward, done, info

        temp_obs = self.get_observations()
        for idx, agent_id in enumerate(self.agent_ids):
            temp_reward[agent_id]["speed"] -= self.reward_baselines[agent_id]["speed"][self.step_count]
            temp_reward[agent_id]["toll"] -= self.reward_baselines[agent_id]["toll"][self.step_count]
        self.step_count += 1
        return temp_obs, \
            temp_reward, \
    ...
    def get_episode_reward_time(self):
        total_veh_t = monstac_api.general.get_total_running_time(self.base_env)
        return total_veh_t

    def get_episode_reward_toll(self):
        return monstac_api.general.get_total_truck_travel_distance(self.base_env)

    def get_reward_baseline(self):
        # temp_action = {}
        # for agent_id in self.agent_ids:
        #     temp_action[agent_id] = 1
        temp_action = [1] * len(self.agent_ids)
        obs = self.reset()
        done = False
        step_count = 0
        while not done:
            # done = self.base_env.step(self.actions)
            done = self.base_env.step(temp_action)
            temp_reward_toll = dict(zip(self.agent_ids, monstac_api.rl.get_rewards(self.base_env)[0]))
            temp_reward_speed = dict(zip(self.agent_ids, monstac_api.rl.get_rewards(self.base_env)[1]))
            for idx, agent_id in enumerate(self.agent_ids):
                self.reward_baselines[agent_id]["speed"].append(temp_reward_speed[agent_id])
                self.reward_baselines[agent_id]["toll"].append(temp_reward_toll[agent_id])
            step_count += 1
        return self.reset()
